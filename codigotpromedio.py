...
for idx, noche in enumerate(noches_separadas):
    ...
    juan = [minutos_bogota[horas_bogota.index(tem)] for tem in noche]

    plt.plot(juan, T_fi, color='thistle')
    
    
    for idx, p in enumerate(juan):
        peras = (p-1020)//5
        noche_sumadas[peras] += T_fi[idx]
        cantidad[peras] +=1
    
noche_promedio = noche_sumadas/cantidad
    
plt.plot(inter,noche_promedio, color="indigo",linewidth=3, label ="Temperatura Promedio")    


plt.xlabel('Hora Local (Bogotá)', weight='bold') 
plt.ylabel(r'$\mathbf{Brillo\ (mag/arcsec^2)}$', weight='bold') 

plt.title('Temperatura General Octubre', weight='bold', fontsize=25, y=1.06)
